...
## Language support (added 2026-09-05, 0.2.0 + 0.2.1 + 0.3.1; Portuguese + pattern cleanup 2026-09-06, 0.3.5; French 2026-09-06, 0.3.10; Russian 2026-09-06, 0.5.8; German 2026-09-06, 0.5.9; Greek 2026-09-06, 0.5.10) — fully done

UI language (English/Spanish/Italian/Portuguese/French/Russian/German/
Greek, top-bar dropdown, 0.2.0 + Italian in 0.3.1 + Portuguese in
0.3.5 + French in 0.3.10 + Russian in 0.5.8 + German in 0.5.9 + Greek
in 0.5.10) — `frontend/src/i18n/` (hand-rolled `en.ts`/`es.ts`/`it.ts`/
`pt.ts`/`fr.ts`/`ru.ts`/`de.ts`/`el.ts`/`index.ts`, no library, `Dict`
type widening so non-English files only have to match English's key
shape, not its exact text). Adding a language is now a proven 6-spot
pattern (confirmed for Italian in 0.3.1, Portuguese in 0.3.5, French
in 0.3.10, Russian in 0.5.8, German in 0.5.9, Greek in 0.5.10 — see
...
German added 0.5.9, Greek added 0.5.10) — `Enricher.language` mirrors
...
Greek each add one line asking for the trivia field in that language
while explicitly protecting `"wiki"`, which must stay the English
Wikipedia article title for `wiki.resolve()`'s lookup).
