...
async def llm_ollama(settings: dict, prompt: str) -> str | None:
    ...
    # — see textutil.py's _GPT_OSS_ANTI_LOOP_RULES comment for the full
    # live-tested diagnosis. That prompt-level fix cuts the failure rate
    # sharply but isn't a 100% guarantee; a higher budget here is cheap
    # insurance for the runs where some deliberation still happens before
    # the model self-corrects, without changing behavior for every other
    # Ollama model (which don't need or get the extra budget).
    num_predict = 2200 if model.startswith("gpt-oss") else 1200
    payload = {
        "model": model,
        "prompt": prompt,
        "stream": False,
