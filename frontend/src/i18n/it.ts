import type { Dict } from './en'

const it: Dict = {
  common: {
    save: 'Salva',
    saving: 'Salvataggio…',
    saved: 'Salvato',
    cancel: 'Annulla',
    delete: 'Elimina',
    add: 'Aggiungi',
    adding: 'Aggiunta…',
  },
  topbar: {
    backToPlayer: 'Torna al lettore',
    settings: 'Impostazioni',
    dashboard: 'Pannello',
    githubProject: 'Pagina del progetto GitHub',
    changePassword: 'Cambia password',
    signOut: 'Esci',
    theme: 'Tema',
    themeDay: 'Giorno',
    themeNight: 'Notte',
    themeSapphire: 'Zaffiro',
    themeJade: 'Giada',
    themeHarbor: 'Porto',
    reloadApp: 'Ricarica app',
  },
  settings: {
    title: 'Impostazioni',
    subtitle: 'Configurazione amministrativa, suddivisa per sezione.',
    backToSettings: '← Impostazioni',
    usersTitle: 'Utenti',
    usersDescription: 'Account, ruoli e nomi visualizzati.',
    aiTitle: 'Provider IA',
    aiDescription: 'OpenCode, Ollama, ChatGPT ed endpoint compatibili con OpenAI.',
    emailTitle: 'Email (SMTP)',
    emailDescription: 'Forgot-password delivery.',
    spotifyTitle: 'Spotify',
    spotifyDescription: 'Per-user playlist integration.',

    deezerTitle: 'Deezer',

    deezerDescription: 'Per-user playlist integration.',
  },
  spotifySettings: {
    title: 'Spotify',
    intro:
      'Client credentials for the Spotify per-user playlist integration. Each listener connects their own ' +
      'Spotify account from the player and gets a private "mradio-web" playlist.',
    credsGroup: 'App credentials',
    clientId: 'Client ID',
    clientSecret: 'Client secret',
    redirectUriNote:
      'The redirect URI and token encryption key are set via environment variables on the server — ' +
      'see the deployment notes.',
    save: 'Save',
    saved: 'Saved',
    saving: 'Saving…',
    errorFallback: 'Could not save settings.',
  },  deezerSettings: {
    title: 'Deezer',
    intro:
      'App credentials for the Deezer per-user playlist integration. Each listener connects their own ' +
      'Deezer account from the player and gets a private "mradio-web" playlist.',
    credsGroup: 'App credentials',
    appId: 'App ID',
    secret: 'Secret key',
    redirectUriNote:
      'The redirect URI and token encryption key are set via environment variables on the server — ' +
      'see the deployment notes.',
    save: 'Salva',
    saved: 'Salvato',
    saving: 'Salvataggio…',
    errorFallback: 'Could not save settings.',
  },

  changePassword: {
    title: 'Cambia password',
    currentPassword: 'Password attuale',
    newPassword: 'Nuova password',
    confirmNewPassword: 'Conferma nuova password',
    tooShort: 'La nuova password deve avere almeno 8 caratteri.',
    mismatch: 'Le password non coincidono.',
    errorFallback: 'Impossibile cambiare la password.',
    submit: 'Imposta password',
    cancel: 'Annulla',
  },
  users: {
    title: 'Utenti',
    subtitle: 'Gli account si creano qui — non c\'è registrazione pubblica.',
    colUsername: 'Nome utente',
    colStatus: 'Stato',
    colCreated: 'Creato',
    pillAdmin: 'admin',
    pillDisabled: 'disabilitato',
    pillPasswordPending: 'password in sospeso',
    makeAdmin: 'Rendi admin',
    removeAdmin: 'Rimuovi admin',
    enable: 'Abilita',
    disable: 'Disabilita',
    resetPassword: 'Reimposta password',
    delete: 'Elimina',
    fieldUsername: 'Nome utente',
    fieldFullName: 'Nome completo',
    fieldEmail: 'Email',
    fieldTempPassword: 'Password temporanea',
    fieldTempPasswordAuto: "Verrà generata automaticamente una password temporanea: l'email di invito permetterà di impostarne una personale.",
    fieldAdmin: 'Admin',
    addUser: 'Aggiungi utente',
    adding: 'Aggiunta…',
    errorFallback: 'Impossibile creare l\'utente.',
    footerNote: 'Gli account senza email devono cambiare la password temporanea al primo accesso.',
    resetPasswordPrompt: 'Nuova password temporanea per {username} (min. 8 caratteri):',
    resetPasswordTooShort: 'La password deve avere almeno 8 caratteri.',
    resetPasswordDone: 'Password reimpostata. Gli verrà chiesto di sceglierne una propria al prossimo accesso.',
    deleteConfirm: 'Eliminare {username}? Questa azione non può essere annullata.',
    editProfile: 'Modifica profilo',
    editFullNamePrompt: 'Nuovo nome completo per {username}:',
    editEmailPrompt: 'Nuova email per {username}:',
    editProfileDone: 'Profilo aggiornato.',
    resendInvite: 'Invia di nuovo invito',
    resendInviteDone: 'Email di invito inviata.',
    resendInviteFailed: "Impossibile inviare l'email di invito.",
  },
  aiSettings: {
    title: 'Provider IA',
    intro:
      'Credenziali condivise per OpenCode, Ollama, ChatGPT e qualsiasi endpoint compatibile con OpenAI (es. NVIDIA NIM). ' +
      'Ogni persona sceglie quale usare dal lettore — nessuno ha bisogno di una propria chiave.',
    ollamaGroup: 'Ollama',
    ollamaNotePrefix: 'Nuovo a Ollama? Vedi',
    ollamaNoteLink: '"Setting up Ollama" nel KB.md',
    ollamaNoteSuffix: 'per come installarlo e scaricare un modello.',
    ollamaModelsLoading: 'Verifica dei modelli installati…',
    ollamaModelsUnreachable: 'Impossibile raggiungere questo server — viene mostrato il modello salvato per ultimo. Salva un URL valido per scegliere tra i modelli effettivamente installati.',
    ollamaLargestModelHint: 'il più grande installato',
    ollamaSizeHintNote: 'La dimensione da sola non predice quale modello risponde meglio — un modello più grande può comunque essere meno preciso su una traccia specifica. Usa Test per confrontare prima di scegliere.',
    serverUrl: 'URL del server',
    model: 'Modello',
    openaiGroup: 'Compatibile con OpenAI (NIM, ecc.)',
    nimNotePrefix: 'Nuovo a NIM? Vedi',
    nimNoteLink: '"Getting an API key" nel KB.md',
    nimNoteSuffix: 'per ottenere una chiave API gratuita.',
    apiBaseUrl: 'URL base API',
    apiKey: 'Chiave API',
    apiKeyNotSet: 'non impostata',
    opencodeGroup: 'OpenCode',
    opencodeEnable: 'Abilita (porta, o "1" per il predefinito)',
    codexGroup: 'Abbonamento ChatGPT / Codex',
    codexIntro:
      'Accedi con un abbonamento ChatGPT Plus, Pro o Go invece di una chiave API. Usa lo stesso accesso ' +
      'della CLI di Codex — non ufficiale e potrebbe smettere di funzionare se OpenAI lo cambia, ma gratuito se paghi già ChatGPT.',
    codexConnect: 'Connetti con ChatGPT',
    codexConnected: 'Connesso ({plan})',
    codexDisconnect: 'Disconnetti',
    codexWaiting: 'In attesa del completamento dell\'accesso…',
    codexUserCodeHint: 'Inserisci questo codice: {code}, oppure apri',
    grokGroup: 'Grok',
    grokIntro:
      "Modelli Grok di xAI. Scegli una chiave API (fatturata per token sul tuo account xAI) oppure il tuo " +
      'abbonamento SuperGrok / X Premium+.',
    grokModeLabel: 'Metodo di accesso a Grok',
    grokModeApiKey: 'Chiave API',
    grokModeSubscription: 'Abbonamento',
    grokConnect: 'Connetti con Grok',
    grokConnected: 'Connesso al tuo abbonamento Grok.',
    geminiGroup: 'Gemini',
    geminiIntro:
      "Modelli Gemini di Google tramite il livello API gratuito — nessuna carta di credito richiesta, " +
      'disponibile a tutti una volta configurato, non riservato agli amministratori come ChatGPT o Grok.',
    geminiNotePrefix: 'Nuovo a Gemini? Vedi',
    geminiNoteLink: '"Getting an API key" nel KB.md',
    geminiNoteSuffix: 'per ottenere una chiave API gratuita.',
    openrouterGroup: 'OpenRouter',
    openrouterIntro:
      'Una chiave, un router di modelli gratuiti — nessuna carta di credito richiesta, disponibile per tutti una ' +
      'volta configurato, non riservato agli admin. Per impostazione predefinita usa il router automatico di OpenRouter, che sceglie sempre un modello attualmente gratuito.',
    openrouterNotePrefix: 'Nuovo su OpenRouter? Consulta',
    openrouterNoteLink: '«Getting an API key» nel KB.md',
    openrouterNoteSuffix: 'per ottenere una chiave API gratuita.',
    mistralGroup: 'Mistral',
    mistralIntro:
      'Il livello gratuito "Experiment" di Mistral AI su La Plateforme — nessuna carta di credito richiesta. ' +
      'Per ora riservato agli admin. Per impostazione predefinita usa ' +
      'un modello più piccolo che rimane entro la quota del livello gratuito (il loro modello di punta non è incluso in questo livello).',
    mistralNotePrefix: 'Nuovo su Mistral? Consulta',
    mistralNoteLink: '«Getting an API key» nel KB.md',
    mistralNoteSuffix: 'per ottenere una chiave API gratuita.',
    dahlGroup: 'DAHL',
    dahlIntro:
      "MiniMax-M2.7 via DAHL's free inference endpoint — no credit card required, first 100M tokens free. " +
      'Research-only for now; enable manually to test.',
    dahlNotePrefix: 'New to DAHL? See',
    dahlNoteLink: '"Getting an API key" in KB.md',
    dahlNoteSuffix: 'for how to get a free API key.',
    providerEnableToggle: '«Mostra nel menu AI del player»',
    providerEnableToggleHint:
      'Disattivalo per nasconderlo dal menu a tendina di tutti senza disconnettere — utile quando raggiungi una quota d’uso.',
    autoHiddenNote:
      'Nascosto temporaneamente dal menu a tendina dopo un errore recente — verrà ritentato automaticamente in background.',
    errorFallback: 'Impossibile salvare le impostazioni.',
    test: 'Prova',
    testing: 'Provando…',
    testSuccess: 'Funziona',
    testFailure: 'Fallito',
    testError: 'Impossibile contattare il server per eseguire la prova.',
  },
  emailSettings: {
    title: 'Email (SMTP)',
    intro:
      "Credenziali per l'invio delle email di reimpostazione password. Una configurazione condivisa per tutta " +
      "l'app — impostata una volta e usata automaticamente ogni volta che qualcuno richiede un link di reimpostazione.",
    kbNotePrefix: 'Vedi',
    kbNoteLink: '"Configuring email" nel KB.md',
    kbNoteSuffix: 'per maggiori dettagli sulla configurazione.',
    gmailGroup: 'Usi Gmail?',
    gmailStep1: "Attiva la verifica in due passaggi sull'account Google da cui vuoi inviare.",
    gmailStep2: 'Apri',
    gmailStep3: "Incolla la password per le app generata (16 caratteri) nel campo Password qui sotto — non la tua normale password Gmail. Host/porta sono smtp.gmail.com / 587, già i valori predefiniti qui.",
    gmailAppPasswordLink: 'Password per le app di Google',
    genericGroup: 'Server SMTP',
    fieldHost: 'Host',
    fieldPort: 'Porta',
    fieldUsername: 'Nome utente',
    fieldPassword: 'Password',
    fieldFromAddress: 'Indirizzo mittente',
    fieldUseTls: 'Usa STARTTLS',
    fieldPublicUrl: 'URL pubblico (opzionale)',
    publicUrlHint:
      "Di solito non serve — il link nelle email di reimpostazione è costruito dal dominio usato da chi ascolta " +
      "per raggiungere l'app. Imposta questo solo se quel rilevamento dovesse mai sbagliare.",
    passwordNotSet: 'non impostata',
    errorFallback: 'Impossibile salvare le impostazioni.',
  },
  stationBrowser: {
    title: 'Le tue stazioni',
    favoriteSlots: '12 posti preferiti',
    favorites: 'Preferiti',
    genres: 'Generi',
    editFavorites: 'Modifica preferiti',
    doneEditing: 'Fine modifica',
    slotEmpty: '— vuoto',
    slotMoveHere: ' · sposta qui',
    removeFavoriteConfirm: 'Rimuovere questo preferito?',
    renameFavorite: 'Rinomina',
    deleteFavorite: 'Elimina',
    saveRename: 'Salva nome',
    cancelRename: 'Annulla',
    addStreamPlaceholder: 'Aggiungi un URL di streaming…',
    add: 'Aggiungi',
    adding: 'Aggiunta…',
    invalidUrl: 'Deve essere un URL di streaming http(s) valido.',
    addedToFavorites: 'Aggiunto ai preferiti.',
    alreadyFavoriteOrFull: 'Già un preferito, oppure nessun posto libero.',
    couldNotAddUrl: 'Impossibile aggiungere quell\'URL.',
    alreadyFavorite: 'Già un preferito.',
    addedNamedToFavorites: '{name} aggiunto ai preferiti.',
    noFreeSlots: 'Nessun posto libero — rimuovine uno prima.',
    editHintPick: 'Tocca una stazione per prenderla, poi tocca un posto per spostarla lì.',
    editHintDrop: 'Tocca un posto per lasciarla lì — tocca di nuovo lo stesso posto per annullare.',
    footer: '{label} · {count} stazioni · preferiti mostrati per primi',
  },
  nowPlaying: {
    ariaLabel: 'In riproduzione',
    nothingPlaying: 'Nessuna riproduzione',
    pickStation: 'Scegli una stazione dai preferiti o dai generi per iniziare ad ascoltare.',
    stopped: 'Fermato — premi play per riconnetterti.',
    connecting: 'Connessione…',
    noIcySupport: 'Questa stazione non supporta i metadati ICY — nessuna informazione sul brano disponibile.',
    linerNotes: 'Note di copertina',
    askingProvider: 'Interrogando il provider IA…',
    askingNamedProvider: 'Interrogando {provider}…',
    noLinerNotes: 'Nessuna nota per questo brano.',
    showMore: 'Mostra di più',
    showLess: 'Mostra di meno',
    readOnWikipedia: 'Leggi su Wikipedia',
    reAskAi: 'Richiedi all\'IA',
    stop: 'Ferma',
    play: 'Riproduci',
    reconnect: 'Riconnetti',
    mute: 'Silenzia',
    unmute: 'Attiva audio',
    volume: 'Volume',
    aiProvider: 'Provider IA',
    musicService: 'Music service',
    historyTitle: 'Recently played',
    openOnService: 'Open on {service}',
    none: 'nessuno',
    selectOne: 'Seleziona uno',
  },
  analytics: {
    title: 'Analitica',
    intro: 'Chi sta ascoltando, cosa sta riproducendo e da dove — da quando questo account ha iniziato a registrare le sessioni.',
    liveTitle: 'In diretta ora',
    liveEmpty: 'Nessuno sta ascoltando in questo momento.',
    mapTitle: 'Mappa degli ascoltatori',
    mapPins: 'Indicatori',
    mapHeatmap: 'Mappa di calore',
    mapScopeLive: 'Solo in diretta',
    mapScopeHistory: 'In diretta + cronologia recente',
    statsTitle: 'Statistiche',
    since7d: 'Ultimi 7 giorni',
    since30d: 'Ultimi 30 giorni',
    sinceAll: 'Sempre',
    topStations: 'Stazioni più ascoltate',
    topGenres: 'Generi più ascoltati',
    topUsers: 'Ascoltatori principali',
    byDay: 'Sessioni al giorno',
    statsEmpty: 'Ancora nessuna sessione in questo intervallo.',
    historyTitle: 'Cronologia recente',
    historyEmpty: 'Ancora nessuna sessione registrata.',
    colUser: 'Utente',
    colStation: 'Stazione',
    colGenre: 'Genere',
    colLocation: 'Posizione',
    colElapsed: 'Trascorso',
    colStarted: 'Iniziato',
    locationLocal: 'rete locale',
    prevPage: 'Precedente',
    nextPage: 'Successivo',
  },
}

export default it
