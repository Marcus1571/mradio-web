...
const ru: Dict = {
  ...
  users: {
    ...
    fieldTempPasswordAuto: 'Временный пароль будет сгенерирован автоматически — письмо с приглашением позволит задать собственный.',
    fieldAdmin: 'Админ',
    addUser: 'Добавить пользователя',
    adding: 'Добавление…',
    errorFallback: 'Не удалось создать пользователя.',
    footerNote: 'Аккаунты без email должны сменить временный пароль при первом входе.',
  ...
  aiSettings: {
    ...
    ollamaModelsLoading: 'Проверка установленных моделей…',
    ollamaModelsUnreachable: 'Не удалось подключиться к этому серверу — показана последняя сохранённая модель. Сохраните действующий URL, чтобы выбирать из фактически установленных моделей.',
    ollamaLargestModelHint: 'самая большая установленная',
    ollamaSizeHintNote: 'Размер сам по себе не показывает, какая модель отвечает лучше — модель побольше может оказаться менее точной на конкретном треке. Используйте «Тест», чтобы сравнить перед выбором.',
    ...
    mistralIntro:
      ...
      'Пока доступен только администраторам. По умолчанию используется модель меньшего ' +
    ...
    autoHiddenNote:
      'Временно скрыт из выпадающего меню после недавней ошибки — автоматически повторная попытка в фоновом режиме.',
