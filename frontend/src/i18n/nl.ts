import type { Dict } from './en'

const nl: Dict = {
  common: {
    save: 'Opslaan',
    saving: 'Opslaan…',
    saved: 'Opgeslagen',
    cancel: 'Annuleren',
    delete: 'Verwijderen',
    add: 'Toevoegen',
    adding: 'Toevoegen…',
  },
  topbar: {
    backToPlayer: 'Terug naar speler',
    settings: 'Instellingen',
    dashboard: 'Dashboard',
    githubProject: 'GitHub-projectpagina',
    changePassword: 'Wachtwoord wijzigen',
    signOut: 'Afmelden',
    theme: 'Thema',
    themeDay: 'Dag',
    themeNight: 'Nacht',
    themeSapphire: 'Saffier',
    themeJade: 'Jade',
    themeHarbor: 'Haven',
    reloadApp: 'App opnieuw laden',
  },
  settings: {
    title: 'Instellingen',
    subtitle: 'Beheerdersinstellingen, per onderdeel gegroepeerd.',
    backToSettings: '← Instellingen',
    usersTitle: 'Gebruikers',
    usersDescription: 'Accounts, rollen en weergavenamen.',
    aiTitle: 'AI-providers',
    aiDescription: 'OpenCode, Ollama, ChatGPT en OpenAI-compatibele endpoints.',
    emailTitle: 'E-mail (SMTP)',
    emailDescription: 'Forgot-password delivery.',
    spotifyTitle: 'Spotify',
    spotifyDescription: 'Per-user playlist integration.',

    deezerTitle: 'Deezer',

    deezerDescription: 'Per-user playlist integration.',
  },
  spotifySettings: {
    title: 'Spotify',
    intro:
      'Client credentials for the Spotify per-user playlist integration. Each listener connects their own ' +
      'Spotify account from the player and gets a private "mradio-web" playlist.',
    credsGroup: 'App credentials',
    clientId: 'Client ID',
    clientSecret: 'Client secret',
    redirectUriNote:
      'The redirect URI and token encryption key are set via environment variables on the server — ' +
      'see the deployment notes.',
    save: 'Save',
    saved: 'Saved',
    saving: 'Saving…',
    errorFallback: 'Could not save settings.',
  },  deezerSettings: {
    title: 'Deezer',
    intro:
      'App credentials for the Deezer per-user playlist integration. Each listener connects their own ' +
      'Deezer account from the player and gets a private "mradio-web" playlist.',
    credsGroup: 'App credentials',
    appId: 'App ID',
    secret: 'Secret key',
    redirectUriNote:
      'The redirect URI and token encryption key are set via environment variables on the server — ' +
      'see the deployment notes.',
    save: 'Opslaan',
    saved: 'Opgeslagen',
    saving: 'Opslaan…',
    errorFallback: 'Could not save settings.',
  },

  changePassword: {
    title: 'Wachtwoord wijzigen',
    currentPassword: 'Huidig wachtwoord',
    newPassword: 'Nieuw wachtwoord',
    confirmNewPassword: 'Bevestig nieuw wachtwoord',
    tooShort: 'Het nieuwe wachtwoord moet minstens 8 tekens bevatten.',
    mismatch: 'Wachtwoorden komen niet overeen.',
    errorFallback: 'Wachtwoord wijzigen mislukt.',
    submit: 'Wachtwoord instellen',
    cancel: 'Annuleren',
  },
  users: {
    title: 'Gebruikers',
    subtitle: 'Accounts worden hier aangemaakt — er is geen openbare registratie.',
    colUsername: 'Gebruikersnaam',
    colStatus: 'Status',
    colCreated: 'Aangemaakt',
    pillAdmin: 'beheerder',
    pillDisabled: 'uitgeschakeld',
    pillPasswordPending: 'wachtwoord in behandeling',
    makeAdmin: 'Beheerder maken',
    removeAdmin: 'Beheerdersrechten intrekken',
    enable: 'Inschakelen',
    disable: 'Uitschakelen',
    resetPassword: 'Wachtwoord resetten',
    delete: 'Verwijderen',
    fieldUsername: 'Gebruikersnaam',
    fieldFullName: 'Volledige naam',
    fieldEmail: 'E-mail',
    fieldTempPassword: 'Tijdelijk wachtwoord',
    fieldTempPasswordAuto: 'Er wordt automatisch een tijdelijk wachtwoord gegenereerd — via de uitnodigingsmail kunnen ze zelf een wachtwoord instellen.',
    fieldAdmin: 'Beheerder',
    addUser: 'Gebruiker toevoegen',
    adding: 'Toevoegen…',
    errorFallback: 'Gebruiker aanmaken mislukt.',
    footerNote: 'Accounts zonder e-mail moeten hun tijdelijke wachtwoord wijzigen bij de eerste aanmelding.',
    resetPasswordPrompt: 'Nieuw tijdelijk wachtwoord voor {username} (min. 8 tekens):',
    resetPasswordTooShort: 'Het wachtwoord moet minstens 8 tekens bevatten.',
    resetPasswordDone: 'Wachtwoord gereset. Bij de volgende aanmelding wordt gevraagd een eigen wachtwoord in te stellen.',
    deleteConfirm: '{username} verwijderen? Dit kan niet ongedaan worden gemaakt.',
    editProfile: 'Profiel bewerken',
    editFullNamePrompt: 'Nieuwe volledige naam voor {username}:',
    editEmailPrompt: 'Nieuw e-mailadres voor {username}:',
    editProfileDone: 'Profiel bijgewerkt.',
    resendInvite: 'Uitnodiging opnieuw versturen',
    resendInviteDone: 'Uitnodigingsmail verzonden.',
    resendInviteFailed: 'Kon de uitnodigingsmail niet verzenden.',
  },
  aiSettings: {
    title: 'AI-providers',
    intro:
      'Gedeelde inloggegevens voor OpenCode, Ollama, ChatGPT en elk OpenAI-compatibel endpoint (bijv. NVIDIA NIM). Iedereen ' +
      'kiest vanuit de speler welke hij gebruikt — niemand heeft een eigen sleutel nodig.',
    ollamaGroup: 'Ollama',
    ollamaNotePrefix: 'Nieuw met Ollama? Zie',
    ollamaNoteLink: '„Setting up Ollama" in KB.md',
    ollamaNoteSuffix: 'voor installatie en het downloaden van een model.',
    ollamaModelsLoading: 'Controleren welke modellen zijn geïnstalleerd…',
    ollamaModelsUnreachable: 'Kon deze server niet bereiken — het laatst opgeslagen model wordt getoond. Sla een geldige URL op om te kiezen uit de daadwerkelijk geïnstalleerde modellen.',
    ollamaLargestModelHint: 'grootste geïnstalleerde',
    ollamaSizeHintNote: 'Grootte alleen zegt niet welk model het beste antwoord geeft — een groter model kan bij een bepaald nummer nog steeds minder nauwkeurig zijn. Gebruik Test om te vergelijken voordat je een model kiest.',
    serverUrl: 'Server-URL',
    model: 'Model',
    openaiGroup: 'OpenAI-compatibel (NIM enz.)',
    nimNotePrefix: 'Nieuw met NIM? Zie',
    nimNoteLink: '„Getting an API key" in KB.md',
    nimNoteSuffix: 'om een gratis API-sleutel te krijgen.',
    apiBaseUrl: 'API-basis-URL',
    apiKey: 'API-sleutel',
    apiKeyNotSet: 'niet ingesteld',
    opencodeGroup: 'OpenCode',
    opencodeEnable: 'Inschakelen (poort, of "1" voor standaard)',
    codexGroup: 'ChatGPT-/Codex-abonnement',
    codexIntro:
      'Log in met een ChatGPT Plus-, Pro- of Go-abonnement in plaats van een API-sleutel. Gebruikt dezelfde login ' +
      'als de Codex CLI — niet officieel en kan stoppen met werken als OpenAI dit wijzigt, maar gratis als je al voor ChatGPT betaalt.',
    codexConnect: 'Verbinden met ChatGPT',
    codexConnected: 'Verbonden ({plan})',
    codexDisconnect: 'Verbinding verbreken',
    codexWaiting: 'Wachten tot inloggen is voltooid…',
    codexUserCodeHint: 'Voer deze code in: {code}, of open',
    grokGroup: 'Grok',
    grokIntro:
      "Grok-modellen van xAI. Kies een API-sleutel (per token gefactureerd op je xAI-account) of je " +
      'SuperGrok-/X Premium+-abonnement.',
    grokModeLabel: 'Grok-aanmeldmethode',
    grokModeApiKey: 'API-sleutel',
    grokModeSubscription: 'Abonnement',
    grokConnect: 'Verbinden met Grok',
    grokConnected: 'Verbonden met je Grok-abonnement.',
    geminiGroup: 'Gemini',
    geminiIntro:
      'Gemini-modellen van Google via hun gratis API-niveau — geen creditcard nodig, beschikbaar voor ' +
      'iedereen zodra ingesteld, niet alleen voor beheerders zoals bij ChatGPT of Grok.',
    geminiNotePrefix: 'Nieuw met Gemini? Zie',
    geminiNoteLink: '„Getting an API key" in KB.md',
    geminiNoteSuffix: 'om een gratis API-sleutel te krijgen.',
    openrouterGroup: 'OpenRouter',
    openrouterIntro:
      'Eén sleutel, een router voor gratis modellen — geen creditcard nodig, beschikbaar voor iedereen zodra ' +
      'ingesteld, niet alleen voor beheerders. Gebruikt standaard OpenRouters eigen auto-router, die altijd een op dat moment gratis model kiest.',
    openrouterNotePrefix: 'Nieuw met OpenRouter? Zie',
    openrouterNoteLink: '„Getting an API key" in KB.md',
    openrouterNoteSuffix: 'om een gratis API-sleutel te krijgen.',
    mistralGroup: 'Mistral',
    mistralIntro:
      'Het gratis "Experiment"-niveau van Mistral AI op La Plateforme — geen creditcard nodig. ' +
      'Voorlopig alleen voor beheerders. Gebruikt standaard een kleiner model dat ' +
      'binnen het quotum van het gratis niveau blijft (hun topmodel is niet inbegrepen op dit niveau).',
    mistralNotePrefix: 'Nieuw met Mistral? Zie',
    mistralNoteLink: '„Getting an API key" in KB.md',
    mistralNoteSuffix: 'om een gratis API-sleutel te krijgen.',
    dahlGroup: 'DAHL',
    dahlIntro:
      "MiniMax-M2.7 via DAHL's free inference endpoint — no credit card required, first 100M tokens free. " +
      'Research-only for now; enable manually to test.',
    dahlNotePrefix: 'New to DAHL? See',
    dahlNoteLink: '"Getting an API key" in KB.md',
    dahlNoteSuffix: 'for how to get a free API key.',
    providerEnableToggle: '‘Weergeven in het AI-menu van de speler’',
    providerEnableToggleHint:
      'Zet uit om het voor iedereen uit het dropdownmenu te verbergen zonder te verbreken — handig zodra een gebruikslimiet is bereikt.',
    autoHiddenNote:
      'Tijdelijk verborgen uit het dropdownmenu na een recente storing — wordt automatisch opnieuw geprobeerd op de achtergrond.',
    errorFallback: 'Instellingen opslaan mislukt.',
    test: 'Testen',
    testing: 'Testen…',
    testSuccess: 'Werkt',
    testFailure: 'Mislukt',
    testError: 'Kan de server niet bereiken om te testen.',
  },
  emailSettings: {
    title: 'E-mail (SMTP)',
    intro:
      "Inloggegevens voor het versturen van wachtwoordherstel-e-mails. Eén gedeelde configuratie voor de hele app — " +
      'eenmalig ingesteld, automatisch gebruikt zodra iemand een resetlink aanvraagt.',
    kbNotePrefix: 'Zie',
    kbNoteLink: '„Configuring email" in KB.md',
    kbNoteSuffix: 'voor meer over deze instelling.',
    gmailGroup: 'Gebruikt u Gmail?',
    gmailStep1: 'Schakel 2-staps-verificatie in voor het Google-account waarmee u wilt versturen.',
    gmailStep2: 'Open',
    gmailStep3: 'Plak het gegenereerde app-wachtwoord van 16 tekens hieronder in het wachtwoordveld — niet uw gewone Gmail-wachtwoord. Host/poort zijn smtp.gmail.com / 587, dit zijn hier al de standaardwaarden.',
    gmailAppPasswordLink: 'Google-app-wachtwoorden',
    genericGroup: 'SMTP-server',
    fieldHost: 'Host',
    fieldPort: 'Poort',
    fieldUsername: 'Gebruikersnaam',
    fieldPassword: 'Wachtwoord',
    fieldFromAddress: 'Afzenderadres',
    fieldUseTls: 'STARTTLS gebruiken',
    fieldPublicUrl: 'Openbare URL (optioneel)',
    publicUrlHint:
      'Meestal niet nodig — de link in reset-e-mails wordt opgebouwd op basis van het domein waarmee de luisteraar ' +
      'de app heeft bereikt. Alleen instellen als die detectie ooit fout gaat.',
    passwordNotSet: 'niet ingesteld',
    errorFallback: 'Instellingen opslaan mislukt.',
  },
  stationBrowser: {
    title: 'Uw zenders',
    favoriteSlots: '12 favorietenplekken',
    favorites: 'Favorieten',
    genres: 'Genres',
    editFavorites: 'Favorieten bewerken',
    doneEditing: 'Klaar met bewerken',
    slotEmpty: '— leeg',
    slotMoveHere: ' · hierheen verplaatsen',
    removeFavoriteConfirm: 'Deze favoriet verwijderen?',
    renameFavorite: 'Hernoemen',
    deleteFavorite: 'Verwijderen',
    saveRename: 'Naam opslaan',
    cancelRename: 'Annuleren',
    addStreamPlaceholder: 'Stream-URL toevoegen…',
    add: 'Toevoegen',
    adding: 'Toevoegen…',
    invalidUrl: 'Moet een geldige http(s) stream-URL zijn.',
    addedToFavorites: 'Toegevoegd aan favorieten.',
    alreadyFavoriteOrFull: 'Al een favoriet, of geen vrije plekken.',
    couldNotAddUrl: 'Kon die URL niet toevoegen.',
    alreadyFavorite: 'Al een favoriet.',
    addedNamedToFavorites: '{name} toegevoegd aan favorieten.',
    noFreeSlots: 'Geen vrije plekken — verwijder er eerst een.',
    editHintPick: 'Tik op een zender om deze te selecteren, tik dan op een plek om hem daarheen te verplaatsen.',
    editHintDrop: 'Tik op een plek om daar neer te zetten — tik nogmaals op dezelfde plek om te annuleren.',
    footer: '{label} · {count} zenders · favorieten eerst getoond',
  },
  nowPlaying: {
    ariaLabel: 'Nu speelt',
    nothingPlaying: 'Er speelt niets',
    pickStation: 'Kies een zender uit favorieten of genres om te beginnen met luisteren.',
    stopped: 'Gestopt — druk op afspelen om opnieuw te verbinden.',
    connecting: 'Verbinden…',
    noIcySupport: 'Deze zender ondersteunt geen ICY-metadata — trackinformatie is niet beschikbaar.',
    linerNotes: 'Notities',
    askingProvider: 'Vraag aan AI-provider…',
    askingNamedProvider: 'Vraag aan {provider}…',
    noLinerNotes: 'Geen notities voor dit nummer.',
    showMore: 'Meer weergeven',
    showLess: 'Minder weergeven',
    readOnWikipedia: 'Lezen op Wikipedia',
    reAskAi: 'AI opnieuw vragen',
    stop: 'Stop',
    play: 'Afspelen',
    reconnect: 'Opnieuw verbinden',
    mute: 'Dempen',
    unmute: 'Dempen opheffen',
    volume: 'Volume',
    aiProvider: 'AI-provider',
    musicService: 'Music service',
    historyTitle: 'Recently played',
    openOnService: 'Open on {service}',
    none: 'geen',
    selectOne: 'Selecteer een',
  },
  analytics: {
    title: 'Analyse',
    intro: 'Wie luistert, wat en vanwaar — sinds dit account sessies bijhoudt.',
    liveTitle: 'Nu live',
    liveEmpty: 'Er luistert nu niemand.',
    mapTitle: 'Luisterkaart',
    mapPins: 'Spelden',
    mapHeatmap: 'Heatmap',
    mapScopeLive: 'Alleen live',
    mapScopeHistory: 'Live + recente geschiedenis',
    statsTitle: 'Statistieken',
    since7d: 'Laatste 7 dagen',
    since30d: 'Laatste 30 dagen',
    sinceAll: 'Sinds het begin',
    topStations: 'Meest beluisterde zenders',
    topGenres: 'Meest beluisterde genres',
    topUsers: 'Grootste luisteraars',
    byDay: 'Sessies per dag',
    statsEmpty: 'Nog geen sessies in deze periode.',
    historyTitle: 'Recente geschiedenis',
    historyEmpty: 'Nog geen sessies geregistreerd.',
    colUser: 'Gebruiker',
    colStation: 'Zender',
    colGenre: 'Genre',
    colLocation: 'Locatie',
    colElapsed: 'Verstreken',
    colStarted: 'Gestart',
    locationLocal: 'lokaal netwerk',
    prevPage: 'Vorige',
    nextPage: 'Volgende',
  },
}

export default nl
