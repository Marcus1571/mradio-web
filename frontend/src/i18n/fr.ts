import type { Dict } from './en'

const fr: Dict = {
  common: {
    save: 'Enregistrer',
    saving: 'Enregistrement…',
    saved: 'Enregistré',
    cancel: 'Annuler',
    delete: 'Supprimer',
    add: 'Ajouter',
    adding: 'Ajout…',
  },
  topbar: {
    backToPlayer: 'Retour au lecteur',
    settings: 'Paramètres',
    dashboard: 'Tableau de bord',
    githubProject: 'Page du projet GitHub',
    changePassword: 'Changer le mot de passe',
    signOut: 'Se déconnecter',
    switchToLight: 'Passer au thème clair',
    switchToDark: 'Passer au thème sombre',
    reloadApp: "Recharger l'application",
  },
  settings: {
    title: 'Paramètres',
    subtitle: 'Configuration administrateur, regroupée par section.',
    backToSettings: '← Paramètres',
    usersTitle: 'Utilisateurs',
    usersDescription: 'Comptes, rôles et noms affichés.',
    aiTitle: "Fournisseurs d'IA",
    aiDescription: 'OpenCode, Ollama, ChatGPT et points de terminaison compatibles OpenAI.',
    emailTitle: 'E-mail (SMTP)',
    emailDescription: 'Envoi des liens de réinitialisation de mot de passe.',
  },
  changePassword: {
    title: 'Changer le mot de passe',
    currentPassword: 'Mot de passe actuel',
    newPassword: 'Nouveau mot de passe',
    confirmNewPassword: 'Confirmer le nouveau mot de passe',
    tooShort: 'Le nouveau mot de passe doit comporter au moins 8 caractères.',
    mismatch: 'Les mots de passe ne correspondent pas.',
    errorFallback: 'Impossible de changer le mot de passe.',
    submit: 'Définir le mot de passe',
    cancel: 'Annuler',
  },
  users: {
    title: 'Utilisateurs',
    subtitle: 'Les comptes sont créés ici — il n\'y a pas d\'inscription publique.',
    colUsername: "Nom d'utilisateur",
    colStatus: 'Statut',
    colCreated: 'Créé le',
    pillAdmin: 'admin',
    pillDisabled: 'désactivé',
    pillPasswordPending: 'mot de passe en attente',
    makeAdmin: 'Nommer admin',
    removeAdmin: "Retirer l'admin",
    enable: 'Activer',
    disable: 'Désactiver',
    resetPassword: 'Réinitialiser le mot de passe',
    delete: 'Supprimer',
    fieldUsername: "Nom d'utilisateur",
    fieldFullName: 'Nom complet',
    fieldEmail: 'E-mail',
    fieldTempPassword: 'Mot de passe temporaire',
    fieldTempPasswordAuto: "Un mot de passe temporaire sera généré automatiquement : l'e-mail d'invitation permettra d'en définir un.",
    fieldAdmin: 'Admin',
    addUser: 'Ajouter un utilisateur',
    adding: 'Ajout…',
    errorFallback: "Impossible de créer l'utilisateur.",
    footerNote: 'Les comptes sans e-mail doivent changer leur mot de passe temporaire à la première connexion.',
    resetPasswordPrompt: 'Nouveau mot de passe temporaire pour {username} (min. 8 caractères) :',
    resetPasswordTooShort: 'Le mot de passe doit comporter au moins 8 caractères.',
    resetPasswordDone: 'Mot de passe réinitialisé. Il sera demandé d\'en définir un nouveau à la prochaine connexion.',
    deleteConfirm: 'Supprimer {username} ? Cette action est irréversible.',
    editProfile: 'Modifier le profil',
    editFullNamePrompt: 'Nouveau nom complet pour {username} :',
    editEmailPrompt: 'Nouvel e-mail pour {username} :',
    editProfileDone: 'Profil mis à jour.',
    resendInvite: "Renvoyer l'invitation",
    resendInviteDone: "E-mail d'invitation envoyé.",
    resendInviteFailed: "Impossible d'envoyer l'e-mail d'invitation.",
  },
  aiSettings: {
    title: "Fournisseurs d'IA",
    intro:
      "Identifiants partagés pour OpenCode, Ollama, ChatGPT et tout point de terminaison compatible OpenAI (ex. NVIDIA NIM). " +
      "Chaque personne choisit lequel utiliser depuis le lecteur — personne n'a besoin de sa propre clé.",
    ollamaGroup: 'Ollama',
    ollamaNotePrefix: 'Nouveau sur Ollama ? Consultez',
    ollamaNoteLink: '« Setting up Ollama » dans le KB.md',
    ollamaNoteSuffix: "pour savoir comment l'installer et obtenir un modèle.",
    ollamaModelsLoading: 'Vérification des modèles installés…',
    ollamaModelsUnreachable: "Impossible de contacter ce serveur — le dernier modèle enregistré est affiché. Enregistrez une URL valide pour choisir parmi les modèles réellement installés.",
    ollamaLargestModelHint: 'le plus volumineux installé',
    ollamaSizeHintNote: "La taille seule ne prédit pas quel modèle répond le mieux — un modèle plus volumineux peut rester moins précis sur un morceau donné. Utilisez Tester pour comparer avant de faire votre choix.",
    serverUrl: 'URL du serveur',
    model: 'Modèle',
    openaiGroup: 'Compatible OpenAI (NIM, etc.)',
    nimNotePrefix: 'Nouveau sur NIM ? Consultez',
    nimNoteLink: '« Getting an API key » dans le KB.md',
    nimNoteSuffix: 'pour obtenir une clé API gratuite.',
    apiBaseUrl: "URL de base de l'API",
    apiKey: "Clé API",
    apiKeyNotSet: 'non définie',
    opencodeGroup: 'OpenCode',
    opencodeEnable: 'Activer (port, ou "1" pour la valeur par défaut)',
    codexGroup: 'Abonnement ChatGPT / Codex',
    codexIntro:
      "Connectez-vous avec un abonnement ChatGPT Plus, Pro ou Go au lieu d'une clé API. Utilise la même connexion " +
      "que la CLI Codex — non officiel et pourrait cesser de fonctionner si OpenAI le modifie, mais gratuit si vous payez déjà ChatGPT.",
    codexConnect: 'Se connecter avec ChatGPT',
    codexConnected: 'Connecté ({plan})',
    codexDisconnect: 'Déconnecter',
    codexWaiting: 'En attente de la fin de la connexion…',
    codexUserCodeHint: 'Entrez ce code : {code}, ou ouvrez',
    grokGroup: 'Grok',
    grokIntro:
      "Modèles Grok de xAI. Choisissez une clé API (facturée au jeton sur votre compte xAI) ou votre " +
      'abonnement SuperGrok / X Premium+.',
    grokModeLabel: 'Méthode de connexion à Grok',
    grokModeApiKey: 'Clé API',
    grokModeSubscription: 'Abonnement',
    grokConnect: 'Se connecter avec Grok',
    grokConnected: 'Connecté à votre abonnement Grok.',
    geminiGroup: 'Gemini',
    geminiIntro:
      "Modèles Gemini de Google via leur offre API gratuite — aucune carte bancaire requise, disponible pour " +
      "tout le monde une fois configuré, pas réservé aux administrateurs comme ChatGPT ou Grok.",
    geminiNotePrefix: 'Nouveau sur Gemini ? Consultez',
    geminiNoteLink: '« Getting an API key » dans le KB.md',
    geminiNoteSuffix: 'pour obtenir une clé API gratuite.',
    openrouterGroup: 'OpenRouter',
    openrouterIntro:
      'Une clé, un routeur de modèles gratuits — aucune carte bancaire requise, accessible à tout le monde une ' +
      'fois configuré, pas réservé aux administrateurs. Utilise par défaut le routeur automatique propre à OpenRouter, qui choisit toujours un modèle actuellement gratuit.',
    openrouterNotePrefix: 'Nouveau sur OpenRouter ? Consultez',
    openrouterNoteLink: '« Getting an API key » dans le KB.md',
    openrouterNoteSuffix: 'pour obtenir une clé API gratuite.',
    mistralGroup: 'Mistral',
    mistralIntro:
      "Le niveau gratuit « Experiment » de Mistral AI sur La Plateforme — aucune carte bancaire requise. " +
      'Réservé aux administrateurs pour le moment. Utilise par défaut un ' +
      'modèle plus petit qui reste dans le quota du niveau gratuit (leur modèle phare n\'est pas inclus dans ce niveau).',
    mistralNotePrefix: 'Nouveau sur Mistral ? Consultez',
    mistralNoteLink: '« Getting an API key » dans le KB.md',
    mistralNoteSuffix: 'pour obtenir une clé API gratuite.',
    providerEnableToggle: '« Afficher dans le menu IA du lecteur »',
    providerEnableToggleHint:
      'Désactivez pour le masquer du menu déroulant de tout le monde sans le déconnecter — pratique en cas de quota d’utilisation atteint.',
    autoHiddenNote:
      'Masqué temporairement du menu déroulant après un échec récent — nouvelle tentative automatique en arrière-plan.',
    errorFallback: "Impossible d'enregistrer les paramètres.",
    test: 'Tester',
    testing: 'Test…',
    testSuccess: 'Fonctionne',
    testFailure: 'Échec',
    testError: "Impossible de contacter le serveur pour exécuter le test.",
  },
  emailSettings: {
    title: 'E-mail (SMTP)',
    intro:
      "Identifiants pour l'envoi des e-mails de réinitialisation de mot de passe. Une configuration partagée pour " +
      "toute l'application — définie une fois, utilisée automatiquement à chaque demande de lien de réinitialisation.",
    kbNotePrefix: 'Voir',
    kbNoteLink: '« Configuring email » dans le KB.md',
    kbNoteSuffix: 'pour en savoir plus sur cette configuration.',
    gmailGroup: 'Vous utilisez Gmail ?',
    gmailStep1: "Activez la validation en deux étapes sur le compte Google depuis lequel vous voulez envoyer.",
    gmailStep2: 'Ouvrez',
    gmailStep3: "Collez le mot de passe d'application généré (16 caractères) dans le champ Mot de passe ci-dessous — pas votre mot de passe Gmail habituel. L'hôte/port sont smtp.gmail.com / 587, déjà les valeurs par défaut ici.",
    gmailAppPasswordLink: "Mots de passe d'application Google",
    genericGroup: 'Serveur SMTP',
    fieldHost: 'Hôte',
    fieldPort: 'Port',
    fieldUsername: "Nom d'utilisateur",
    fieldPassword: 'Mot de passe',
    fieldFromAddress: 'Adresse d\'expédition',
    fieldUseTls: 'Utiliser STARTTLS',
    fieldPublicUrl: 'URL publique (optionnel)',
    publicUrlHint:
      "Généralement inutile — le lien dans les e-mails de réinitialisation est construit à partir du domaine " +
      "utilisé par la personne pour accéder à l'application. Ne renseignez ceci que si cette détection venait à échouer.",
    passwordNotSet: 'non défini',
    errorFallback: "Impossible d'enregistrer les paramètres.",
  },
  stationBrowser: {
    title: 'Vos stations',
    favoriteSlots: '12 emplacements favoris',
    favorites: 'Favoris',
    genres: 'Genres',
    editFavorites: 'Modifier les favoris',
    doneEditing: 'Terminer',
    slotEmpty: '— vide',
    slotMoveHere: ' · déplacer ici',
    addStreamPlaceholder: "Ajouter une URL de flux…",
    add: 'Ajouter',
    adding: 'Ajout…',
    invalidUrl: 'Doit être une URL de flux http(s) valide.',
    addedToFavorites: 'Ajouté aux favoris.',
    alreadyFavoriteOrFull: "Déjà en favori, ou plus d'emplacement libre.",
    couldNotAddUrl: "Impossible d'ajouter cette URL.",
    alreadyFavorite: 'Déjà en favori.',
    addedNamedToFavorites: '{name} ajouté aux favoris.',
    noFreeSlots: "Aucun emplacement libre — supprimez-en un d'abord.",
    editHintPick: 'Touchez une station pour la sélectionner, puis un emplacement pour l\'y déplacer.',
    editHintDrop: 'Touchez un emplacement pour l\'y déposer — touchez-le à nouveau pour annuler.',
    footer: '{label} · {count} stations · favoris affichés en premier',
    removeFavoriteConfirm: 'Retirer ce favori ?',
    renameFavorite: 'Renommer',
    deleteFavorite: 'Supprimer',
    saveRename: 'Enregistrer le nom',
    cancelRename: 'Annuler',
  },
  nowPlaying: {
    ariaLabel: 'En cours de lecture',
    nothingPlaying: 'Rien en cours de lecture',
    pickStation: 'Choisissez une station parmi les favoris ou les genres pour commencer à écouter.',
    stopped: 'Arrêté — appuyez sur lecture pour vous reconnecter.',
    connecting: 'Connexion…',
    noIcySupport: "Cette station ne prend pas en charge les métadonnées ICY — aucune information sur le morceau n'est disponible.",
    linerNotes: 'Notes de pochette',
    askingProvider: "Interrogation du fournisseur d'IA…",
    askingNamedProvider: 'Interrogation de {provider}…',
    noLinerNotes: 'Aucune note pour ce morceau.',
    showMore: 'Afficher plus',
    showLess: 'Afficher moins',
    readOnWikipedia: 'Lire sur Wikipédia',
    reAskAi: "Redemander à l'IA",
    stop: 'Arrêter',
    play: 'Lecture',
    reconnect: 'Reconnecter',
    mute: 'Muet',
    unmute: 'Activer le son',
    volume: 'Volume',
    aiProvider: "Fournisseur d'IA",
    none: 'aucun',
    historyTitle: 'Écoutés récemment',
  },
  analytics: {
    title: 'Analyses',
    intro: "Qui écoute, quoi et d'où — depuis que ce compte enregistre des sessions.",
    liveTitle: 'En direct maintenant',
    liveEmpty: "Personne n'écoute en ce moment.",
    mapTitle: "Carte des auditeurs",
    mapPins: 'Repères',
    mapHeatmap: 'Carte de chaleur',
    mapScopeLive: 'En direct uniquement',
    mapScopeHistory: 'En direct + historique récent',
    statsTitle: 'Statistiques',
    since7d: '7 derniers jours',
    since30d: '30 derniers jours',
    sinceAll: 'Depuis toujours',
    topStations: 'Stations les plus écoutées',
    topGenres: 'Genres les plus écoutés',
    topUsers: 'Principaux auditeurs',
    byDay: 'Sessions par jour',
    statsEmpty: "Pas encore de sessions dans cette plage.",
    historyTitle: 'Historique récent',
    historyEmpty: "Pas encore de sessions enregistrées.",
    colUser: 'Utilisateur',
    colStation: 'Station',
    colGenre: 'Genre',
    colLocation: 'Emplacement',
    colElapsed: 'Écoulé',
    colStarted: 'Démarré',
    locationLocal: 'réseau local',
    prevPage: 'Précédent',
    nextPage: 'Suivant',
  },
}

export default fr
