...
**Spotify** — attempted a live Client Credentials token exchange using the app's own already-configured `spotify_client_id`/`spotify_client_secret` (saved on the production install from the earlier, now-shelved OAuth integration work). Got `400 invalid_client: Invalid client secret` — **this is a credential problem with this specific installation's saved secret, not a flaw in the mechanism**. Plausible causes, not diagnosed further since it's out of scope for validating the concept: the saved secret predates a Spotify-side rotation, was never the right value to begin with (the OAuth flow that used it went through several real bugs per `STATUS.md`'s v1.17.x history), or a transcription issue. **Needs a fresh, verified Client ID/Secret pair (or a live re-check of the existing one on Spotify's dashboard) before implementation** — the Client Credentials *flow itself* is well-documented, exempt from the user cap per multiple independent sources, and used by essentially every third-party Spotify search integration that exists; there's no reason to doubt the mechanism, only this one saved value.
...
- **A working Spotify Client ID/Secret pair.** The one saved on the production install returned `invalid_client` in this session's live test — needs a fresh pair from [developer.spotify.com/dashboard](https://developer.spotify.com/dashboard) (the existing Premium-owned app used for the shelved OAuth feature, or a new one) verified against the Client Credentials token endpoint specifically before any code is written against it.
